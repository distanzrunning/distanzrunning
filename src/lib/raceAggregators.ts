...
4b. "surface_breakdown" — one of "100% Paved" | "Unpaved" | "Mixed", ONLY when a page states the course's actual surface composition. Asphalt, tarmac, concrete, cobblestones, setts, pavé, and brick are ALL paved surfaces — a course made only of these is "100% Paved" (Rome's cobblestone sections do not make it Mixed). "Unpaved" = only gravel/dirt/grass/sand/trail. "Mixed" ONLY when the course combines paved AND unpaved sections. Null when no page describes the surface itself — do NOT infer it from the race type.
