...
// ============================================================================
// Clear-button helper used across demos
// ============================================================================
...
function ClearButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      ...
      className="flex size-5 cursor-pointer items-center justify-center rounded text-textDefault transition-colors hover:bg-[color:var(--ds-gray-200)]"
    >
...
const defaultCode = `import CollapsibleInput from '@/components/ui/CollapsibleInput';
...
export function Component(): JSX.Element {
  ...
  return (
    <CollapsibleInput
      ...
      collapsedAriaLabel="Search"
      aria-label="Search"
    />
  );
}`;

const sizesCode = `import CollapsibleInput from '@/components/ui/CollapsibleInput';
import { Search } from 'lucide-react';
import { useState } from 'react';
import type { JSX } from 'react';
...
const clearButtonCode = `import CollapsibleInput from '@/components/ui/CollapsibleInput';
...
const customWidthCode = `import CollapsibleInput from '@/components/ui/CollapsibleInput';
import { Search } from 'lucide-react';
import { useState } from 'react';
import type { JSX } from 'react';
...
function DefaultDemo() {
  const [value, setValue] = useState("");
  return (
    <div className="flex items-start justify-start flex-initial">
      <CollapsibleInput
        prefix={<Search className="size-4" />}
        placeholder="Search"
        value={value}
...
function SizesDemo() {
  ...
  const [large, setLarge] = useState("");
  return (
    <div className="flex flex-col md:flex-row items-start justify-start gap-4 flex-initial">
      <CollapsibleInput
        size="small"
        prefix={<Search className="size-4" />}
        placeholder="Small"
        value={small}
        onChange={(e) => setSmall(e.target.value)}
        collapsedAriaLabel="Search small"
      />
      <CollapsibleInput
        size="default"
        prefix={<Search className="size-4" />}
        ...
        value={base}
        onChange={(e) => setBase(e.target.value)}
        collapsedAriaLabel="Search default"
      />
      <CollapsibleInput
        size="large"
        prefix={<Search className="size-4" />}
        ...
        value={large}
        onChange={(e) => setLarge(e.target.value)}
        collapsedAriaLabel="Search large"
      />
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col items-start justify-start gap-4 flex-initial">
      <CollapsibleInput
        ...
        value={narrow}
        onChange={(e) => setNarrow(e.target.value)}
        collapsedAriaLabel="Search narrow"
        expandedWidth={180}
      />
      <CollapsibleInput
        prefix={<Search className="size-4" />}
        ...
        value={wide}
        onChange={(e) => setWide(e.target.value)}
        collapsedAriaLabel="Search wide"
        expandedWidth={360}
      />
    </div>
  );
    ...
    <>
      ...
      <Section>
        ...
        >
          ...
          &apos;s scale so the two can sit alongside each other in
          the same row.
        </p>
      ...
      <Section>
        <SectionHeader id="clear-button" onCopyLink={showToast}>
          Clear button
        </SectionHeader>
        <p
          className="text-copy-16 text-textSubtle mt-3 mb-6"
        ...
        <CodePreview componentCode={clearButtonCode}>
          <ClearButtonDemo />
        </CodePreview>
      </Section>
