...
const TableOfContentsWidget: React.FC<TableOfContentsWidgetProps> = ({ body }) => {
  ...
  useEffect(() => {
    // Early return if body is null, undefined, or empty
    ...
    body.forEach((block) => {
      if (block && block._type === 'block' && (block.style === 'h1' || block.style === 'h2' || block.style === 'h3')) {
  ...
  // Don't render if no headings, too few headings, or body is empty/invalid
  if (!body || body.length === 0 || headings.length < 2) {
    return null;
  }

  // Additional check: don't render if all headings are too short or just whitespace
  const validHeadings = headings.filter(heading => 
    heading.text.trim().length > 2 && 
    heading.text.trim() !== 'undefined' &&
    !heading.text.match(/^[\s\-_]+$/) // Not just spaces, dashes, or underscores
  );
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed left-8 top-1/2 transform -translate-y-1/2 z-50 bg-surface border border-gray-200 rounded-full p-3 shadow-lg hover:shadow-xl transition-all duration-200 hover:bg-gray-50"
      ...
      {/* Widget Panel - Floating panel on left side */}
      <div 
        className={`fixed left-8 top-1/2 transform -translate-y-1/2 z-40 transition-all duration-300 ease-in-out ${
          isOpen ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0'
        }`}
        style={{ width: '320px', maxHeight: '80vh' }}
      >
        <nav className="bg-surface border border-gray-200 rounded-lg shadow-xl flex flex-col overflow-hidden">
          {/* Header */}
