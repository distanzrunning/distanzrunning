...
|---|---|---|---|---|
| **Canvas** (down) | `bg-canvas` | `#F5F5F5` | `#121212` | the page / `PageFrame` / app-shell wrappers, **and** recessed sub-areas *within* a surface: modal & drawer footers/sections |
| **Surface** (up) | `bg-surface` | `#FFFFFF` | `#262626` | **everything raised, including all form controls**: cards, menus, popovers, dialogs, drawers, sheets, panels, toasts, badges, chips, filled/secondary buttons, **inputs, textareas, selects, date pickers**, checkbox/radio/switch boxes, the header pill |
...
**Surface ≠ its own states.** `surface` is its **own `#262626`** (decoupled from `bg-200`, the `#1F1F1F` page-wrapper tone), and `gray-100/200/300` (`#303030/#3A3A3A/#444`) are the **hover/active/selected** tones one step above it — kept distinct so a hover/selected child *on* a surface doesn't collapse into it (the ThemeSwitcher bug). **Segmented controls** share one pattern: track = `bg-canvas` (recessed), selected segment = `bg-surface` (raised) + a hairline ring — see `ThemeSwitcher`/`Switch`.
