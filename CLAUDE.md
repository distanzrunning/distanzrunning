...
- **Editorial radius = 4px** (`rounded-xs`) — ONE radius for editorial images **and** editorial card surfaces (ArticleCard, RaceCard, NewsletterSignup card; user call 2026-07-29, deliberately squarer than Vercel/Quartr's stock 8px). Focus rings on these surfaces match (`rounded-xs`). The remapped `rounded-lg` (12px) remains the **material** radius for floating chrome only (menus, modals — Geist-verbatim `material-*`), not for editorial cards or bare images. For future article-body images, Vercel also frames them: `border gray-alpha-400` + `--ds-shadow-small` + surface bg.
- **Header rules are full-bleed** (user calls 2026-07-29 + 2026-08-01): the masthead's two horizontal rules (top-tier divider + nav row bottom rule) AND the mega-menu panel (Radix Viewport canvas + bottom rule) span the full viewport width; header *content* (nav links, mega-menu columns — and page content generally) stays on the `max-w-content` column unless a unit is explicitly full-bleed (PromoUnit band).
