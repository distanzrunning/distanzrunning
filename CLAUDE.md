...
- **Header rules are full-bleed** (user call 2026-07-29): the masthead's two horizontal rules (top-tier divider + nav row bottom rule) span the full viewport width; header *content* (and page content generally) stays on the `max-w-content` column unless a unit is explicitly full-bleed (PromoUnit band).
